...
class DatabasePostgresConnection
{
    /** @var ?resource */
    private $connection;
    ...
    public function __construct($connection, $socket)
    {
        ...
        $poll = static function ($watcher) use (&$deferred, &$connection) {
            if (!\pg_consume_input($connection)) {
                ...
                if ($deferred !== null) {
                    $deferred->fail(new DatabasePostgresException(\pg_last_error()));
                }
                return;
            }
            if ($deferred === null) {
    ...
    public static function connect(string $dsn): Promise
    {
        if (!$connection = @ \pg_connect($dsn, \PGSQL_CONNECT_ASYNC | \PGSQL_CONNECT_FORCE_NEW)) {
            return new Failure(new DatabasePostgresException(self::CONNECTION_ERROR));
        }
        if (\pg_connection_status($connection) === \PGSQL_CONNECTION_BAD) {
            return new Failure(new DatabasePostgresException(\pg_last_error($connection)));
        }
        if (!$socket = \pg_socket($connection)) {
            return new Failure(new DatabasePostgresException(self::SOCKET_ERROR));
        }
        ...
        $callback = function ($watcher, $socket) use ($connection, $deferred) {
            switch (\pg_connect_poll($connection)) {
                ...
                case \PGSQL_POLLING_FAILED:
                    $deferred->fail(new DatabasePostgresException(\pg_last_error($connection)));
                    return;
    ...
    private function send(callable $function, ...$args): \Generator
    {
        ...
        if ($this->connection === null) {
            throw new DatabasePostgresException(self::DISCONNECT_ERROR);
        }
    ...
    private function createResult($result)
    {
        switch (\pg_result_status($result, \PGSQL_STATUS_LONG)) {
            ...
            case \PGSQL_BAD_RESPONSE:
                throw new DatabasePostgresException(\pg_result_error($result));

            case \PGSQL_EMPTY_QUERY:
                throw new DatabasePostgresException(self::EMPTY_QUERY_ERROR);

            default:
                throw new DatabasePostgresException(self::UNKNOWN_RESULT_ERROR);
        }
    }
